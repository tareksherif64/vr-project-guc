...
public class NPCController : MonoBehaviour
{
    ...
    void Update()
    {
        animator.SetBool("CorrectItem", deliveryZone.correctItem);
        animator.SetBool("CashTaken", (cashRegister.cashTaken | cardReader.cardTaken));

        AnimatorStateInfo state = animator.GetCurrentAnimatorStateInfo(0);

        // Reset CorrectItem once transition away from Standing Idle 0 has started
        ...
        // Reset CashTaken once transition away from Standing Idle 0 0 has started
        if (animator.IsInTransition(0) && (cashRegister.cashTaken || cardReader.cardTaken))
        {
            cashRegister.cashTaken = false;
            cardReader.cardTaken = false;
            animator.SetBool("CashTaken", false);
        }
